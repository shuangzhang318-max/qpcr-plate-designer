...
const App = () => {
  ...
  const calculateBestOrientation = useCallback((n) => {
    // 1. 如果能被8整除（如2,4,8），竖排是绝对完美的（契合8联管）
    if (8 % n === 0) return 'vertical';
    // 2. 如果能被12整除（如3,6），横排能让行宽更整齐
  ...
  useEffect(() => {
    ...
    setGroupConfigs({});
    setSelectedGroupIds([]);
  }, [samples.length, targets.length, replicates, priority, calculateBestOrientation]);
  ...
  const autoLayout = useCallback(() => {
    let tempWells = Array(96).fill(null);
    const n = replicates;
    ...
    let virtualWells = Array(96).fill(null);
    let maxColUsed = 0;
        ...
        for (let y = 0; y <= 8 - h; y++) {
          ...
          for (let dy = 0; dy < h; dy++) {
            for (let dx = 0; dx < w; dx++) {
              if (wellArray[(y + dy) * 12 + (x + dx)] !== null) { canFit = false; break; }
            }
            if (!canFit) break;
          }
          if (canFit) {
            for (let dy = 0; dy < h; dy++) {
              for (let dx = 0; dx < w; dx++) {
                wellArray[(y + dy) * 12 + (x + dx)] = unit.id;
    ...
    const offsetCols = Math.max(0, Math.floor((12 - (maxColUsed + 1)) / 2));

    units.forEach(unit => {
      const config = groupConfigs[unit.id] || { orientation: globalOrientation };
      const w = config.orientation === 'vertical' ? 1 : n;
      const h = config.orientation === 'vertical' ? n : 1;
      for (let x = offsetCols; x <= 12 - w; x++) {
        for (let y = 0; y <= 8 - h; y++) {
          let canFit = true;
          for (let dy = 0; dy < h; dy++) {
            for (let dx = 0; dx < w; dx++) {
              if (tempWells[(y + dy) * 12 + (x + dx)] !== null) { canFit = false; break; }
            }
            if (!canFit) break;
          }
          if (canFit) {
            for (let dy = 0; dy < h; dy++) {
              for (let dx = 0; dx < w; dx++) {
                tempWells[(y + dy) * 12 + (x + dx)] = { ...unit, orientation: config.orientation };
              }
            }
            return;
          }
  ...
  }, [samples, targets, replicates, priority, groupConfigs, globalOrientation]);
  ...
  const activeCols = useMemo(() => {
    ...
    return any ? { start: first, end: last } : { start: 4, end: 7 };
  }, [wells]);
  ...
  const utilization = useMemo(() => Math.round((wells.filter(w => w).length / 96) * 100), [wells]);

  const exportToSVG = () => {
    const startCol = activeCols.start;
    const endCol = activeCols.end;
    const colCount = endCol - startCol + 1;
    const cellSize = 80;
    const padding = 50;
    const svgWidth = (colCount + 1) * cellSize + padding * 2;
    const svgHeight = 9 * cellSize + padding * 2;
    let svg = `<svg width="${svgWidth}" height="${svgHeight}" xmlns="http://www.w3.org/2000/svg">`;
    svg += `<rect width="100%" height="100%" fill="#f8fafc" />`;
    for(let i = 0; i < colCount; i++) svg += `<text x="${padding + (i + 1) * cellSize + cellSize/2}" y="${padding + 30}" font-family="Arial" font-weight="bold" font-size="14" fill="#94a3b8" text-anchor="middle">${startCol + i + 1}</text>`;
    for(let j = 0; j < 8; j++) svg += `<text x="${padding + 20}" y="${padding + (j + 1) * cellSize + cellSize/2 + 5}" font-family="Arial" font-weight="bold" font-size="16" fill="#94a3b8" text-anchor="middle">${String.fromCharCode(65 + j)}</text>`;
    for(let r = 0; r < 8; r++) {
      for(let c = 0; c < colCount; c++) {
        const wellIdx = r * 12 + (startCol + c);
        const well = wells[wellIdx];
        const x = padding + (c + 1) * cellSize;
        const y = padding + (r + 1) * cellSize;
        if (well) {
          svg += `<rect x="${x + 5}" y="${y + 5}" width="${cellSize - 10}" height="${cellSize - 10}" rx="10" fill="${targetColorMap[well.target]}" stroke="#ffffff" stroke-width="2" />`;
          svg += `<text x="${x + cellSize/2}" y="${y + cellSize/2 - 5}" font-family="Arial" font-weight="bold" font-size="10" fill="white" text-anchor="middle">${well.target}</text>`;
          svg += `<text x="${x + cellSize/2}" y="${y + cellSize/2 + 15}" font-family="Arial" font-weight="bold" font-size="10" fill="white" text-anchor="middle" opacity="0.9">${well.sample}</text>`;
        } else {
          svg += `<rect x="${x + 5}" y="${y + 5}" width="${cellSize - 10}" height="${cellSize - 10}" rx="10" fill="#ffffff" stroke="#e2e8f0" stroke-dasharray="4" />`;
        }
      }
    }
    svg += `</svg>`;
    ...
    link.download = `qPCR_Layout_${new Date().getTime()}.svg`;
    link.click();
  };
  ...
  const handleDrop = (targetIdx) => {
    ...
    const sAnchorRow = Math.floor(dragAnchorIdx / 12);
    const sAnchorCol = dragAnchorIdx % 12;
    const tAnchorRow = Math.floor(targetIdx / 12);
    const tAnchorCol = targetIdx % 12;
    const sourceIndices = [];
    wells.forEach((w, idx) => {
    ...
    sourceIndices.forEach(sIdx => {
      const sRow = Math.floor(sIdx / 12);
      const sCol = sIdx % 12;
      const tRow = tAnchorRow + (sRow - sAnchorRow);
      const tCol = tAnchorCol + (sCol - sAnchorCol);
      if (tRow < 0 || tRow > 7 || tCol < 0 || tCol > 11) isOutOfBound = true;
      else mapping.push({ sIdx, tIdx: tRow * 12 + tCol });
    });
          ...
          <div className="flex items-center gap-3 bg-slate-50 p-1.5 rounded-[1.5rem] border border-slate-100 shadow-inner">
             <div className="flex bg-white rounded-xl shadow-sm p-0.5 border border-slate-200">
                ...
                <button onClick={() => setViewMode('mobile')} className={`p-2 rounded-lg transition-all flex items-center gap-2 text-xs font-bold ${viewMode === 'mobile' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>
                   <Smartphone className="w-4 h-4" /> 手机
                </button>
             </div>
          </div>
        </div>
            ...
            <div className={`flex ${isMobile ? 'flex-col items-center gap-6 text-center' : 'justify-between items-center'} mb-10`}>
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center justify-center gap-2 mt-2">
                  ...
                  <div className="text-[10px] font-black px-4 py-1.5 bg-indigo-50 text-indigo-700 rounded-full border border-indigo-100">默认 {globalOrientation === 'vertical' ? '竖向' : '横向'}</div>
                </div>
              </div>
            ...
            >
              <div className="overflow-x-auto max-w-full pb-6 scrollbar-hide" onClick={(e) => e.stopPropagation()}>
                <div 
                  ...
                  style={{ 
                    gridTemplateColumns: `3rem repeat(${activeCols.end - activeCols.start + 1}, ${isMobile ? '4.2rem' : '4.8rem'})` 
                  }}
                >
                  <div className="h-10"></div>
                  {Array.from({ length: activeCols.end - activeCols.start + 1 }).map((_, i) => (
                    <div key={i} className="text-center text-xs font-black text-slate-300 h-10 flex items-center justify-center uppercase tracking-widest bg-slate-200/20 rounded-2xl">
                  ...
                  {Array.from({ length: 8 }).map((_, rowIndex) => (
                    <React.Fragment key={rowIndex}>
                      <div className="flex items-center justify-center text-2xl font-black text-slate-200 h-16 mr-3 uppercase italic">
                        {String.fromCharCode(65 + rowIndex)}
                      </div>
                      {Array.from({ length: activeCols.end - activeCols.start + 1 }).map((_, i) => {
                        const colIdx = activeCols.start + i;
                        const idx = rowIndex * 12 + colIdx;
                        ...
                        return (
                          <div
                            ...
                            className={`relative border-2 rounded-[1.2rem] flex flex-col items-center justify-center cursor-pointer transition-all duration-300 shadow-sm
                              ${well ? 'border-white hover:scale-105' : 'bg-white/50 border-dashed border-slate-200 opacity-20'}
                              ${isSelected ? 'ring-[6px] ring-indigo-500 ring-offset-4 z-10 scale-110 border-indigo-500 shadow-indigo-200 shadow-2xl' : 'ring-1 ring-slate-100'}
                              ${isDragging ? 'opacity-50 border-indigo-300' : ''}
                            `}
                            style={{ 
                              backgroundColor: well ? targetColorMap[well.target] : 'transparent',
                              width: isMobile ? '4.2rem' : '4.8rem',
                              height: isMobile ? '4.2rem' : '4.8rem',
                            }}
                          >
                            {well && (
                              <div className="text-center w-full px-1 overflow-hidden select-none">
                                <div className="text-[10px] font-black truncate leading-tight text-white tracking-tighter drop-shadow-md uppercase">{well.target}</div>
                                <div className="text-[10px] text-white font-black truncate leading-tight mt-1 tracking-tighter drop-shadow-md">{well.sample}</div>
                              </div>
                            )}
                          </div>
                        );
