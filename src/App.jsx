...
const COPY = {
  zh: { plate96: '96孔板', plate384: '384孔板', configuration: '配置中心', primerFocused: '引物集中', sampleFocused: '样本集中', reverseOrientation: '一键反转全局复孔方向', replicates: '复孔数量 (n)', primerList: '引物清单', sampleList: '样本清单', reactionSetup: '体系配制', totalPerWell: '单孔总体积', excess: '损耗余量 (%)', addComponent: '添加组分', utilization: '利用率', mode: '模式', default: '默认', vertical: '竖向', horizontal: '横向', selectedGroup: '选中组', selectedGroups: (count) => `${count} 组已选`, batchEditing: '批量编辑中', insufficientWells: '孔位不足', overflow: (count) => `有 ${count} 组复孔无法放下，请尝试切换复孔方向，必要时减少样本或引物。`, exportPng: '导出 PNG', resetLayout: '初始化方案', setupGuide: '体系指南', totalMixVolume: (loss) => `总配液体积 (含${loss}%损耗)`, waiting: '等待数据', tips: '操作提示', tip1: '切换到384孔板模式时，建议使用大屏幕设备查看。', tip2: '点击目标加样孔可选中一组复孔，并单独调整复孔方向；按住 Ctrl/Command 键点击可实现多选并批量调节方向。', tip3: '结构化交换：拖拽已选中孔位，可整体移动并保持其内部横纵结构不变。', tip4: '修改配置后，系统会重置自定义排版并重新执行智能方向决策。', language: '语言选择', title: 'qPCR 智能布板工具', seoTitle: 'qPCR 智能布板工具 | 96 孔和 384 孔板设计', seoDescription: '免费的 qPCR 智能布板工具，支持 96 孔和 384 孔板设计、样本与引物复孔自动排布、孔位方向调整、反应体系总体积计算和 PNG 布局导出，帮助减少手工布板错误。', ogDescription: '设计 96 孔和 384 孔 qPCR 板布局，组织样本、引物、复孔和反应体系。' },
  en: { plate96: '96-well', plate384: '384-well', configuration: 'Configuration', primerFocused: 'Primer-focused', sampleFocused: 'Sample-focused', reverseOrientation: 'Reverse all replicate orientations', replicates: 'Replicates (n)', primerList: 'Primer list', sampleList: 'Sample list', reactionSetup: 'Reaction Setup', totalPerWell: 'Total volume per well', excess: 'Excess allowance (%)', addComponent: 'Add component', utilization: 'Utilization', mode: 'mode', default: 'Default', vertical: 'Vertical', horizontal: 'Horizontal', selectedGroup: 'Selected group', selectedGroups: (count) => `${count} groups selected`, batchEditing: 'Batch editing', insufficientWells: 'Insufficient wells', overflow: (count) => `${count} replicate group(s) could not be placed. Try changing orientation or reducing samples or primers.`, exportPng: 'Export PNG', resetLayout: 'Reset layout', setupGuide: 'Reaction Setup Guide', totalMixVolume: (loss) => `Total reaction volume (${loss}% excess)`, waiting: 'Waiting for data', tips: 'Tips', tip1: 'Use a large display when working with a 384-well plate.', tip2: 'Select a replicate group by clicking a populated well, then adjust its orientation. Hold Ctrl/Command while clicking to select multiple groups.', tip3: 'Structured exchange: drag selected wells to move the complete group while preserving its orientation.', tip4: 'Changing the configuration resets custom placement and reruns smart orientation.', language: 'Language selector', title: 'qPCR Smart Layout', seoTitle: 'qPCR Plate Layout Designer | 96-Well and 384-Well Plate Planner', seoDescription: 'Free qPCR plate layout designer for 96-well and 384-well plates. Organize samples, primers, replicates, and reaction volumes, then export your layout as a PNG.', ogDescription: 'Plan qPCR plate layouts, organize samples and primers, set replicates, calculate reaction volumes, and export a PNG.' }
};
...
const App = () => {
  ...
  useEffect(() => {
    ...
    document.getElementById('og-url')?.setAttribute('content', language === 'en'
      ? 'https://qpcr-plate-designer.unwedomain.xyz/?lang=en'
      : 'https://qpcr-plate-designer.unwedomain.xyz/');
